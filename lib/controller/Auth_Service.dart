...
class AuthService {
  ...
  Future<String?> registerUser(UserAccount user) async {
    ...
    try {
      ...
      return UserAccount.fromJson(doc.data() as Map<String, dynamic>); // Success, return user data
    } on FirebaseAuthException catch (e) {
      switch (e.code) {
        case 'user-not-found':
  ...
  static Future<void> _subscribeToTopics(String userId) async {
    ...
    try {
      ...
      if (userDoc.exists) {
        String role = userDoc.get('role') ?? 'user'; // Default to 'user' if role is not set
